...
#Modificamos formato del campo expiring_date para que tenga formato de tipo DATE en una nueva columna y luego eliminamos la columna original
## pero antes desactivamos el modo seguro de actualización y al final de la consulta lo volvemos a activar para poder modificar la tabla
SET SQL_SAFE_UPDATES = 0;
ALTER TABLE credit_card ADD expiring_date_ok DATE;
UPDATE credit_card SET expiring_date_ok = str_to_date(expiring_date, '%m/%d/%y');
ALTER TABLE credit_card DROP COLUMN expiring_date;
ALTER TABLE credit_card CHANGE expiring_date_ok expiring_date DATE;
SET SQL_SAFE_UPDATES = 1;
SELECT * FROM credit_card;

#N1.Exercici 2: error en el número de compte de l'usuari amb ID CcU-2938. actualitzar la informació ingressada en l'IBAN. La informació que ha de mostrar-se per a aquest registre és: R323456312213576817699999
#verificamos que hay error
SELECT id, iban
FROM credit_Card
WHERE id = 'CcU-2938';
...
#N1.Exercici 3: En la taula "transaction" ingressa un nou usuari amb la següent informació:
...
#N1.Exercici 4: eliminar la columna "pan" de la taula credit_*card.
...
#N2.Exercici 2: crear una vista anomenada VistaMarketing que contingui la següent informació: Nom de la companyia. Telèfon de contacte. País de residència. Mitjana de compra realitzat per cada companyia. Presenta la vista creada, ordenant les dades de major a menor mitjana de compra
CREATE VIEW VistaMarketing AS
SELECT c.company_name, c.phone, c.country, AVG(t.amount) AS mitj_compra_compania
FROM company AS c
INNER JOIN transaction AS t
ON c.id=t.company_id
WHERE declined = 0
GROUP BY company_name, phone, country
ORDER BY mitj_compra_compania DESC;

SELECT * FROM VistaMarketing;

#N2.Exercici 3: Filtra la vista VistaMarketing per a mostrar només les companyies que tenen el seu país de residència en "Germany"
SELECT * 
FROM VistaMarketing
...
###corroboramos inserción de datos
SELECT * FROM user;


###Cambiamos el nombre del campo email a personal_email
SET SQL_SAFE_UPDATES = 0;
ALTER TABLE user  
